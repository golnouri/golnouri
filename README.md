<div dir="rtl">

## معرفی فارسی

من **مجتبی گل نوری** هستم، مهندس نرم‌افزار، تحلیلگر و توسعه‌دهنده سامانه‌های نرم‌افزاری.

علاقه من به دنیای فناوری و برنامه‌نویسی سال‌ها پیش از ورود به دانشگاه آغاز شد. از همان دوران، تحلیل سیستم‌ها، حل مسائل پیچیده و طراحی نرم‌افزارهای کاربردی برای من جذابیت ویژه‌ای داشت. این علاقه به مرور زمان به مسیر حرفه‌ای زندگی من تبدیل شد. 
در حال حاضر در زمینه تحلیل، طراحی و توسعه نرم‌افزار فعالیت می‌کنم و مدیریت پلتفرم «سوکت» را بر عهده دارم. در طول سال‌های فعالیت حرفه‌ای، تجربه طراحی و پیاده‌سازی انواع سامانه‌های نرم‌افزاری، وب‌اپلیکیشن‌ها و راهکارهای اختصاصی را کسب کرده‌ام. 
اگر برای ایده، کسب‌وکار یا پروژه نرم‌افزاری خود به مشاوره، تحلیل، طراحی یا توسعه نیاز دارید، خوشحال می‌شوم در کنار شما باشم.

</div>




# About Mojtaba Golnouri
<img src="http://socketapp.ir/img/banner/socketapp-index.png" width="100%" alt="Mojtaba Golnouri">

I'm **Mojtaba Golnouri**, a Software Engineer, Systems Analyst, and Software Developer with over a decade of experience in designing and developing software solutions. My passion for technology and programming started long before university. From an early age, I was fascinated by system analysis, solving complex problems, and creating practical software. Over time, this passion became my professional career. Today, I work as a software analyst, designer, and developer, and I am also the founder and manager of the **Socket Platform**. Throughout my career, I have designed and developed various web applications, business systems, and custom software solutions for different needs and industries. If you are looking for consultation, system analysis, software design, or application development for your business or project, I would be glad to help.

## Projects

### B2B Platform

* Developed using ASP.NET technologies
* Business-to-business platform with custom features

### Ministry of Education Website

* Web-based application
* Development and maintenance of educational services

## Skills

* C#
* ASP.NET Core
* Blazor
* JavaScript & jQuery
* SQL Server
* MongoDB

## Services

* Web Application Development
* Website Design and Development
* Systems Analysis and Software Architecture
* Windows Desktop Application Development
* Android Application Development

## Contact

* GitHub: https://github.com/golnouri
* Email: [golnouri@gmail.com](mailto:golnouri@gmail.com)
* Phone: +98 912 314 6296

---

